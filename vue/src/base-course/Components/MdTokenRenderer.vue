<template>
  ...
  <p class="headline" v-else-if="token.type === 'paragraph'">
    <md-token-renderer v-for='(subTok, j) in token.tokens' :key='j' :token='subTok' :last="last && token.tokens.length === 1" />
  </p>
  ...
  <table v-else-if="token.type ==='table'" :align="token.align" >
    <thead><th v-for="(h, j) in token.header" :key="j">{{h}}</th></thead>
    <tbody>
...
export default class MdTokenRenderer extends Vue {
  ...
  public isComponent(token: marked.Token) { return isComponent(token); }
  public containsComponent(token: marked.Token) { return containsComponent(token); }
  public splitTextToken(token: marked.Tokens.Text) { return splitTextToken(token); }

  public parsedComponent(token: marked.Tokens.Em): {
    is: string,
    text: string
  } {
    return {
